...
export default function LoginScreen({ onLogin }) {
  ...
  const handleSocial = (provider) => {
    setError("");
    setLoading(provider);
    // Simulate OAuth handshake — replace with real SDK calls here
    setTimeout(() => finish({
      name:  provider === "google" ? "Lilly" : "Lilly",
      email: provider === "google" ? "lilly@gmail.com" : "lilly@icloud.com",
      provider,
    }), 1500);
  };
      ...
      <div style={{ padding:"0 24px", flex:1 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:11, marginBottom:20, animation:"fadeUp 0.5s ease 0.05s both" }}>
          ...
          <button
            ...
            disabled={!!loading}
            style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:12, width:"100%", padding:"14px 16px", borderRadius:14, border:"1.5px solid #dadce0", background:"#fff", cursor:loading?"default":"pointer", opacity:loading&&loading!=="google"?0.5:1, transition:"all 0.2s", boxShadow:"0 1px 6px #0000001a", position:"relative" }}
          >
          ...
          <button
            ...
            disabled={!!loading}
            style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:12, width:"100%", padding:"14px 16px", borderRadius:14, border:"1.5px solid #333", background:"#000", cursor:loading?"default":"pointer", opacity:loading&&loading!=="apple"?0.5:1, transition:"all 0.2s", position:"relative" }}
          >
        ...
        <div style={{ animation:"fadeUp 0.5s ease 0.15s both" }}>
          {mode === "signup" && (
            <Field label="YOUR NAME" value={name} onChange={v=>{setName(v);setError("");}} placeholder="e.g. Lilly" />
          )}
        ...
        <button
          onClick={handleEmail}
          disabled={!!loading}
          style={{ width:"100%", padding:"15px", borderRadius:15, border:"none", cursor:loading?"default":"pointer", background:emailValid?`linear-gradient(135deg,${G.green},${G.greenDark})`:G.bg3, color:emailValid?"#fff":G.textMuted, fontSize:14, fontFamily:"Georgia,serif", fontWeight:700, boxShadow:emailValid?`0 8px 24px ${G.greenGlow}`:"none", transition:"all 0.3s", marginBottom:14, display:"flex", alignItems:"center", justifyContent:"center", gap:8, opacity:loading&&loading!=="email"?0.5:1 }}
        >
        ...
        {/* Guest */}
        <button
          onClick={() => !loading && onLogin({ name:"Lilly", email:"", provider:"guest" })}
